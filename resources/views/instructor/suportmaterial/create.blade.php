<x-layouts.dashboard>
    ...
    <div class="uk-card uk-card-default uk-card-small uk-card-body">
        <form class="p-4 pt-0"
            action="{{ route('instructor.courses.support_materials.store', ['course' => $course->id, 'module' => $module, 'lesson' => $lesson]) }}"
            method="POST">
                        ...
                        <div class="uk-position-relative w-100">
                            <select class="selectpicker" name="material_type">
                                @foreach ($material_types as $key => $material_type)
                                    <option value="{{ $material_type['id'] }}" @selected(old('material_type') === $material_type['id'] || $key === 0)>
