<x-layouts.dashboard>
    ...
    <div class="uk-card uk-card-default uk-card-small uk-card-body">
        <form class="p-4 pt-0"
            action="{{ route('instructor.courses.questions.store', ['course' => $course->id, 'module' => $module->id, 'lesson' => $lesson->id, 'quiz' => $quiz]) }}"
            method="POST">
                        ...
                        <div class="uk-position-relative w-100">
                            <select class="selectpicker" name="type">
                                @foreach ($question_types as $key => $question_type)
                                    <option value="{{ $question_type['id'] }}" @selected(old('type') === $question_type['id'] || $key === 0)>
